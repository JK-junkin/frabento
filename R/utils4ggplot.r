...
#' @param first 1個目の値を空白にしない, Default: TRUE
#' @param last 最後の値を空白にしない, Default: TRUE
...
#' if(interactive()){
#'  repl2blank(1:10)
#'  repl2blank(1:10, first = FALSE)
#'  repl2blank(1:10, last = TRUE, inverse = TRUE)
#'  repl2blank(1:10, last = TRUE, inverse = TRUE, first = FALSE)
#'  repl2blank(seq(0, 350, by = 50))
#'  repl2blank(seq(0, 350, by = 50), 2)
#'  repl2blank(seq(0, 350, by = 50), 2, inverse = TRUE)
#'  repl2blank(seq(0, 350, by = 50), 2, inverse = TRUE, last = FALSE)
#'  }
#' }
#' @rdname repl2blank
#' @export
repl2blank <-
    function(vec, nskip = 1, first = TRUE, last = TRUE, inverse = FALSE) {
    i1 <- vec[1]
    index <- seq_along(vec)
    resid <- index[index %% (nskip + 1) == 0] + 1
    if (inverse) {
        vec[index %in% resid] <- ""
        ifelse(first, vec[1] <- "", vec)
    } else {
        vec[!(index %in% resid)] <- ""
        ifelse(first, vec[1] <- i1, vec)
    }
    if (!last) vec[length(vec)] <- ""
    vec
}
...
#' @export
#' @importFrom scales force_all
label_repl2blank <-
    function(nskip = 1, first = TRUE, last = TRUE, inverse = FALSE,
